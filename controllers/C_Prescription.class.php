...
//        }
        ...
        $pdf->ezText(str_pad($p->patient->get_pubpid(), 10, "0", STR_PAD_LEFT) . '-' . str_pad($_SESSION['encounter'], 10, "0", STR_PAD_LEFT), 10);
        $pdf->ezColumnsStop();
        if ($my_y < $pdf->y) {
            $pdf->ezSetY($my_y);
        }
...
//        $pdf->ezText("\n\n" . xl('Signature') . ":________________________________\n" . xl('Date') . ": " . date('Y-m-d'), 12);
	...
	do {
        ...
        $pLastName = $p->patient->lname; //modified by epsdky for prescription filename change to include patient name and ID
        $pLName = convert_safe_file_dir_name($pLastName);
        $modedFileName = "Rx_{$pLName}_{$p->patient->id}_{$_SESSION['encounter']}.pdf";
